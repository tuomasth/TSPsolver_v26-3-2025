...
public class EvolutionaryParts {
    ...
    public static List selectWhatWillBePairedThenCrossingOverAndMutation(List instances, List logicStackCopiesOfOriginals,
        ...
        if(Math.random() < probabilityOfCrossingOver) {
            ...
            for(int i = 0; i < originalInstanceSize -1; i+=2) {
                ...
                if(newC.getLength() < TSP_Solver_UEF.getShortestLengthOfTheOriginalChromosomes()) {
    ...
    public static double calculateFitness(List instances, double wantedPercentsAddedTo100_Min1_Max3) {
        ...
        if(TSP_Solver_UEF.getWantedTourLength() > bestFitness * wantedPercentsAddedTo100_Min1_Max3) {
            TSP_Solver_UEF.setWantedTourLength(bestFitness * wantedPercentsAddedTo100_Min1_Max3);
            System.out.println("----- NEW FITNESS REQUIREMENT: " + TSP_Solver_UEF.getWantedTourLength());
            return TSP_Solver_UEF.getWantedTourLength(); // tour length in other words, less than that means terminating 
        }
        else {
            return TSP_Solver_UEF.getWantedTourLength();
        }
    }
