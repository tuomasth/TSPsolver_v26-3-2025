...
public class Chromosome {
    ...
    /**
     * Form a chromosome, making a TSP tour in other words. Called when creating chromosomes. 
     * Using the SOM-CH-NN algorithm.
     * 
    ...
    public static ArrayList formChromosome_SOM_CH_NN(
            ...
            double learningRate) {
                ...
                neuronCoordinateXs, neuronCoordinateYs, false, null, learningRate); // not yet using logic stacks and their fragments 
    ...
    public static double[][] formChromosome_OPT_NNH_CHH_CHRI(String text, String ERRORMSG,
        ...
        result1 = TSP_Solver_UEF_241908.NearestNeighbour_Algorithm(text); //    call NNH   (1) 
        result2 = TSP_Solver_UEF_241908.ConvexHull_Algorithm(text);       //    call CHH   (2) 
        result3 = TSP_Solver_UEF_241908.Christofides_Algorithm(text);     //    call CHRI  (3) 
        resultE = TSP_Solver_UEF_241908.NearestNeighbour_Algorithm(text); //    call NNH again
        ...
        TSP_Solver_UEF_241908.setOPT3preResult("\tNNH for a couple of times:  " + tour1 + "\n\t     " + length1d + "\n" + 
        ...
        switch(bestOneOfTheTours) {
            case(1): {
                TSP_Solver_UEF_241908.addToOPT3preResult("NNH was chosen for improving purposes."
                        + " Please note that not always the solution will be improved!");
                edges = Sub_algorithms.MakeOptMoves(max, length1d, nnh_coordinates);
                    TSP_Solver_UEF_241908.setWantedTourLength(length1d * TSP_Solver_UEF_241908.getTerminationPercentRequirement()); 
                    TSP_Solver_UEF_241908.setShortestLengthOfTheOriginalChromosomes(length1d);
                break;
            }
            case(2): {
                TSP_Solver_UEF_241908.addToOPT3preResult("CHH was chosen for improving purposes."
                        + " Please note that not always the solution will be improved!");
                edges = Sub_algorithms.MakeOptMoves(max, length2d, chh_coordinates);
                    TSP_Solver_UEF_241908.setWantedTourLength(length2d * TSP_Solver_UEF_241908.getTerminationPercentRequirement()); 
                    TSP_Solver_UEF_241908.setShortestLengthOfTheOriginalChromosomes(length2d);
                break;
            }
            case(3): {
                TSP_Solver_UEF_241908.addToOPT3preResult("CHRI was chosen for improving purposes."
                        + " Please note that not always the solution will be improved!");
                edges = Sub_algorithms.MakeOptMoves(max, length3d, chri_coordinates);
                    TSP_Solver_UEF_241908.setWantedTourLength(length3d * TSP_Solver_UEF_241908.getTerminationPercentRequirement()); 
                    TSP_Solver_UEF_241908.setShortestLengthOfTheOriginalChromosomes(length3d);
                break;
            }
        }
        return edges;
