...
public class TSP_Solver_UEF {
    ...
    static int wantedPopulationSize = 15; // at the start, can change later like other evol. variables
    static int wantedPopulationSizeOriginal;
    static double terminationPercentRequirement = 1.05; // % from best-known opt allowed to survive terminating, 1 = 0%, 1.05 = 5% etc.
        ...
        if(inputresult.equals("ok") && max >=4 && EUC_2D && 
            ...
            for(int e = 0; e < wantedEvolutionIterations; e++) {
                for(int chro = 0; chro < wantedPopulationSize; chro++) {
                    ...
                    if(e > 1 && wantedPopulationSize < 15) { // can be adjusted however wanted, arbitrary values
                        tempC.setChromosomePoints(solutionArray);
                        instances.add(tempC);
                        logicStackCopiesOfOriginals.add(tempC.getStack()); // stacks might be changed
                    }
                ...
                // terminating:
                for(int chrom = 0; chrom < instances.size(); chrom++) {
                    ArrayList c_coordinates = ((Chromosome) instances.get(chrom)).getChromosomePoints();
                    double eucDistOfChrom = 0;
                    for(int j = 0; j < ((Chromosome) instances.get(chrom)).getChromosomePoints().size()-2; j+=2) {
                    ...
                    if(eucDistOfChrom > wantedTourLength) { 
                        ...
                        if((instances.size() > 5) && (((Chromosome) instances.get(chrom)).getLength() > wantedTourLength)) { 
                            System.out.println("\nTERMINATING " + chrom + " with length " + eucDistOfChrom + 
                                    ", wantedTourLength: " + wantedTourLength);
                            Chromosome.terminateChromosome((Chromosome) instances.get(chrom));
                            instances.remove(instances.get(chrom));
                            logicStackCopiesOfOriginals.remove(logicStackCopiesOfOriginals.get(chrom));
                            chrom--;
