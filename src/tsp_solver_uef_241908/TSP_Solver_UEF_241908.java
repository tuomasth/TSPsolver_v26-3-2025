...
public class TSP_Solver_UEF_241908 {
    ...
    static int maxSomIterations = 10; // because of SOM-CH-NN, this should be more than 0 
    static double somLearningRate = 0.1;
    ...
    static int wantedPopulationSizeOriginal = 20;
    static double terminationPercentRequirement = 1.2; // % from best-known opt allowed to survive terminating, 1 = 0%, 1.05 = 5% etc.
    static int wantedEvolutionIterations = 3;
    static double probabilityOfCrossingOver = 0.3;
    static double wantedMutationRate = 0.005;
    static double elastic_band_limit = 1.05;
    static int wantedLogicStackSize = 4;
    static double movingPercentageForFragments = 0.01;
     ...
     * The accuracy can depend on how the vertices are stored and read in the memory, in which order the nodes are located. 
     * 
     * @param input String
     * @return String
     */
     ...
     * optimum solution for very large inputs, my apologies. 
     * 
     * @param input String
     * @return String
     */
        ...
        if(inputresult.equals("ok") && max >=4 && EUC_2D && 
            ...
            */
            ...
            somResultArray = KohonenSOM.performSOM(maxSomIterations, maxPossibleDistanceInHullAvgs, 
                        ...
                        neuronCoordinateXs, neuronCoordinateYs, false, null, somLearningRate); // no logic stacks 
    ...
    public static String OPT3_Algorithm(String text) {
        ...
        if(inputresult.equals("ok") && max >=4 && EUC_2D && 
            ...
            if(hamiltonian) {
                long endTime = System.nanoTime();
                System.out.println("After OPT3 - Time (millisec): " + (endTime - startTime)/1000000);
                System.out.println("After OPT3 - Time (sec, rounded down): " + (endTime - startTime)/1000000000 + "\n");
        ...
        if(inputresult.equals("ok") && max >=4 && EUC_2D && 
            ...
            for(int e = 0; e < wantedEvolutionIterations; e++) {
                for(int chro = 0; chro < wantedPopulationSize; chro++) {
                    ...
                    somResultArray = KohonenSOM.performSOM(maxSomIterations, maxPossibleDistanceInHullAvgs, 
                                inputCoordinateXs, inputCoordinateYs, 
                                neuronCoordinateXs, neuronCoordinateYs, true, (Chromosome) instances.get(chro), somLearningRate);
                ...
                wantedTourLength = EvolutionaryParts.calculateFitness(instances, terminationPercentRequirement); 
                // the percentRequirement can be edited, 1.2 means 20% from the known best one, 
                // 1.5 means 50% from the known best one and so on
                
                
                if(instances.size() > wantedPopulationSizeOriginal*1.4) {
                    // population is getting too large, stricter requirements 
                    wantedTourLength = wantedTourLength * 0.99;
                    System.out.println("\nA BIT STRICTER REQUIREMENTS because of overpopulating: " + wantedTourLength);
                }
                if(instances.size() > wantedPopulationSizeOriginal*1.8) {
                    // population is getting too large, stricter requirements 
                    wantedTourLength = wantedTourLength * 0.97;
                    System.out.println("\nSTRICT REQUIREMENTS because of overpopulating: " + wantedTourLength);
                }
                if(instances.size() > wantedPopulationSizeOriginal*2.2) {
                    // population is getting too large, stricter requirements 
                ...
                for(int chro = 0; chro < instances.size(); chro++) {
                    ...
                    if(eucDistOfChrom > wantedTourLength 
                        ...
                        if((instances.size() > 2) && (c.getLength() > wantedTourLength)) { 
                        // do not delete anything in the population if less than 3 
            ...
            for(int j = 0; j < instances.size(); j++) {
                ...
                if(tourLength < currentBest) {
                    ...
                    for(int k = 0; k < solutionArrayListTEMP.size(); k++) {
                        ...
                        //System.out.println("Choosing the best of the remaining population... new best: " + currentBest);
                    }
                }
            }
