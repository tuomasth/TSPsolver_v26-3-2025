 ...
 * when not moving towards the SOM goal. In practice, this is a simple integer ID stack, nothing too complex when used correctly. 
 * 
...
public class Logic_Stack {
    ...
    private final int[] DATA; // DATA has integers, which are actually IDs that 
                              ...
                              // some would be "use Christofides and move towards 5th clockwise" and so on.
                              // The neurons move by calling a method and then traversing.
    // Interesting series of movements are set in something like [3,14,15,92,6,5] and       // numbers are random
    ...
    // For example all of a same types could be at range 1-4 and not at 1,3,5,7. 
    // This way, one type of a mutation does not cause too weird behaviour all of a sudden with high probability 
    // when the mutation is like "add 1" or "subtract 1" from/to the nearest.
