 ...
 * - Euclidean distance #2 squared, a bit less computing effort, though Java is a slowpoke compared to C++ (for example) 
 * - Convex hull for limiting the space with a "rubber band"
 * - Counter-clockwise turn checking
 * (- Smaller or bigger angle checking, does function, not used)
 ...
 * - Christofides matching #2, use when a good matching cannot be found with #1
 * - Checking the Christofides matching edge coordinates (a little assisting method)
 * - Nearest neighbour sprout
 * - Simple 2-opt moves, reordering nodes
 * (- 3-opt moves or n-opts, not implemented, not functioning, just an idea)
 * - OPT move heuristic, "Lin-Kernighan-kind-of", pushes nodes to a stack and pops them while trying out new connections
 * 
 * 
...
public class Sub_algorithms {
     ...
     * Might still have bugs (not really affecting the TSP tours). 
     * 
    ...
    public static ArrayList Matching(ArrayList x_coordinates, ArrayList y_coordinates) {
        ...
        /*if(x_coordinates.size() > 150) { 
                // if there are more than 150 match nodes, do not try any complex computations 
    ...
    /**
     * Does not function, just an idea. Removing 3 edges, try reconnecting in every possible way.
     * 
    ...
    public static ArrayList threeOpt(ArrayList <Double>coordinates, double eucDistOld) {
        ...
        for(int i = 0; i < coordinates.size()-12; i++) {
            for(int j = 0; j < 7; j++) {
                if(j == 0) { // 3 to 5, 4 to 0 
                    /*Collections.swap(coordinates, i+6, i+10);
                    Collections.swap(coordinates, i+7, i+11);
                    Collections.swap(coordinates, i+8, i+0);
                    Collections.swap(coordinates, i+9, i+1);*/
                }
                if(j == 1) { // 1 to 3, 2 to 4 
                    /*Collections.swap(coordinates, i+2, i+6);
                    Collections.swap(coordinates, i+3, i+7);
                    Collections.swap(coordinates, i+4, i+8);
                    Collections.swap(coordinates, i+5, i+9);*/
                }
                if(j == 2) { // 1 to 5, 2 to 0 
                    /*Collections.swap(coordinates, i+2, i+10);
                    Collections.swap(coordinates, i+3, i+11);
                    Collections.swap(coordinates, i+4, i+0);
                    Collections.swap(coordinates, i+5, i+1);*/
                }
                if(j == 3) { // 1 to 5, 4 to 2, 3 to 0 
                    /*Collections.swap(coordinates, i+2, i+10);
                    Collections.swap(coordinates, i+3, i+11);
                    Collections.swap(coordinates, i+8, i+4);
                    Collections.swap(coordinates, i+9, i+5);
                    Collections.swap(coordinates, i+6, i+0);
                    Collections.swap(coordinates, i+7, i+1);*/
                }
                if(j == 4) { // 1 to 4, 5 to 3, 2 to 0 
                    /*Collections.swap(coordinates, i+2, i+8);
                    Collections.swap(coordinates, i+3, i+9);
                    Collections.swap(coordinates, i+10, i+6);
                    Collections.swap(coordinates, i+11, i+7);
                    Collections.swap(coordinates, i+4, i+0);
                    Collections.swap(coordinates, i+5, i+1);*/
                }
                if(j == 5) { // 1 to 3, 2 to 5, 4 to 0 
                    /*Collections.swap(coordinates, i+2, i+6);
                    Collections.swap(coordinates, i+3, i+7);
                    Collections.swap(coordinates, i+4, i+10);
                    Collections.swap(coordinates, i+5, i+11);
                    Collections.swap(coordinates, i+8, i+0);
                    Collections.swap(coordinates, i+9, i+1);*/
                }
                if(j == 6) { // 1 to 4, 5 to 2, 3 to 0 
                    /*Collections.swap(coordinates, i+2, i+8);
                    Collections.swap(coordinates, i+3, i+9);
                    Collections.swap(coordinates, i+10, i+4);
                    Collections.swap(coordinates, i+11, i+5);
                    Collections.swap(coordinates, i+6, i+0);
                    Collections.swap(coordinates, i+7, i+1);*/
                }
                ...
                if(eucNew < eucDistOld) {
                    ...
                    System.out.println(j + ", IMPROVED RETURNED FROM 3OPT: " + coordinates);
                    return coordinates;
                }
            }
        }
     ...
     * 
     * This implementation is not exactly like in official books (the Lin-Kernighan heuristic). 
    ...
    public static double[][] MakeOptMoves(int max, double eucDistOld, ArrayList coordinates) {
        ...
        for(int i = 0; i < 2000000; i++) { // for loop can adjust how many tries 
            ...
            while(!x_coordinatesSt.isEmpty() && !y_coordinatesSt.isEmpty()) {
                ...
                if(!done) {
                    switc = Math.random() < 0.6; // random boolean, can be "unfairly" balanced on purpose, 
        ...
        for(int i = 0; i < 20000; i++) {    // can adjust how many times 
